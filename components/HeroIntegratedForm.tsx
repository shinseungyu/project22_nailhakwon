...
export default function HeroIntegratedForm() {
  ...
  return (
    <>
      ...
      <div className="w-full bg-brand-gradient rounded-[2rem] p-4 lg:p-5 shadow-[0_20px_40px_-15px_rgba(251,113,133,0.35)] text-left">
        <div className="flex flex-col gap-3 items-stretch">
          
          {/* Inputs Grid - Row-based layout */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            
            {/* 1. Name & Gender */}
            <div className="flex bg-white rounded-full pl-5 pr-2 py-2 shadow-sm items-center transition-shadow focus-within:ring-2 focus-within:ring-white/30">
              <input
                type="text"
            ...
            {/* 2. Birth */}
            <div className="flex bg-white rounded-full px-5 py-3.5 shadow-sm items-center transition-shadow focus-within:ring-2 focus-within:ring-white/30">
              <input
                type="text"
            ...
            <div className="flex gap-2">
              <div className="bg-white rounded-full pl-5 pr-8 py-3.5 shadow-sm flex items-center relative shrink-0 w-[105px] transition-shadow focus-within:ring-2 focus-within:ring-white/30">
                <select
                  value={form.mobile1}
              ...
              <div className="flex-1 bg-white rounded-full px-5 py-3.5 shadow-sm flex items-center transition-shadow focus-within:ring-2 focus-within:ring-white/30">
                <input
                  type="text"
                  value={form.mobile2}
            ...
            {/* 4. Region */}
            <div className="flex bg-white rounded-full px-5 py-3.5 shadow-sm items-center relative transition-shadow focus-within:ring-2 focus-within:ring-white/30">
              <select
                value={form.region}
            ...
            {/* 5. License */}
            <div className="flex bg-white rounded-full px-5 py-3.5 shadow-sm items-center justify-between transition-shadow focus-within:ring-2 focus-within:ring-white/30">
              <span className="text-[15px] font-medium text-stone-400">자격증 보유 여부</span>
              <button 
                type="button"
                onClick={() => set("has_license", form.has_license === "Y" ? "N" : "Y")}
                className="flex items-center gap-2 outline-none group shrink-0"
              >
                <span className={`text-[14px] font-bold transition-colors w-8 text-right ${form.has_license === "Y" ? "text-brand" : "text-stone-400"}`}>
                  {form.has_license === "Y" ? "보유" : "없음"}
                </span>
                <div className={`relative w-11 h-6 rounded-full transition-colors duration-300 shadow-inner ${form.has_license === "Y" ? "bg-brand" : "bg-stone-200 group-hover:bg-stone-300"}`}>
            ...
            <button
              ...
              disabled={submitted}
              className="w-full bg-stone-900 text-white rounded-full text-[17px] font-bold shadow-lg hover:bg-stone-800 active:scale-[0.98] transition-all duration-200 py-3.5 disabled:opacity-50 flex items-center justify-center gap-1.5 leading-none"
            >
              상담 신청하기
            </button>
